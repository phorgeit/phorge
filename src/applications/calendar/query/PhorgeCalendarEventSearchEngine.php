...
final class PhabricatorCalendarEventSearchEngine
  ...
  private function buildCalendarMonthView(
    ...
    $now_year = phorge_format_local_time($now, $viewer, 'Y');
    $now_month = phorge_format_local_time($now, $viewer, 'm');
    $now_day = phorge_format_local_time($now, $viewer, 'j');
  ...
  private function getDisplayYearAndMonthAndDay(
    ...
    if ($this->calendarYear && $this->calendarMonth) {
      ...
      if ($display == 'month') {
        ...
        $day = phorge_format_local_time($epoch, $viewer, 'd');
      }
      $start_year = phorge_format_local_time($epoch, $viewer, 'Y');
      $start_month = phorge_format_local_time($epoch, $viewer, 'm');
      $start_day = $day;
    }
