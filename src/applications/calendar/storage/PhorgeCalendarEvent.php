  ...
  public function renderEventDate(
    ...
    if ($this->getIsAllDay()) {
      if ($show_end_date) {
        return pht(
          '%s - %s, All Day',
          phorge_date($min_epoch, $viewer),
          phorge_date($max_epoch, $viewer));
      } else {
        return pht(
          '%s, All Day',
          phorge_date($min_epoch, $viewer));
      ...
      return pht(
        '%s - %s',
        phorge_datetime($min_epoch, $viewer),
        phorge_datetime($max_epoch, $viewer));
    } else if ($show_end) {
      return pht(
        '%s - %s',
        phorge_datetime($min_epoch, $viewer),
        phorge_time($max_epoch, $viewer));
    } else {
      return pht(
        '%s',
        phorge_datetime($min_epoch, $viewer));
...
/* -(  PhabricatorConduitResultInterface  )---------------------------------- */
  ...
  private function getConduitDateTime($datetime) {
    ...
    return array(
      'epoch' => (int)$epoch,
      'display' => array(
        'default' => phorge_datetime($epoch, $viewer),
      ),
