...
final class PhorgePhorgeAuthProvider
  ...
  public function readFormValuesFromProvider() {
    $config = $this->getProviderConfig();
    $uri = $config->getProperty(self::PROPERTY_PHORGE_URI);

    return parent::readFormValuesFromProvider() + array(
      self::PROPERTY_PHORGE_NAME => $this->getProviderDomain(),
      self::PROPERTY_PHORGE_URI  => $uri,
    );
  }
  ...
  public function readFormValuesFromRequest(AphrontRequest $request) {
    ...
    if ($is_setup) {
      $parent_values = array();
      $name = $request->getStr(self::PROPERTY_PHORGE_NAME);
    } else {
    ...
    return $parent_values + array(
      self::PROPERTY_PHORGE_NAME => $name,
      self::PROPERTY_PHORGE_URI =>
        $request->getStr(self::PROPERTY_PHORGE_URI),
    );
  }
  ...
  public function processEditForm(
    ...
    $key_name = self::PROPERTY_PHORGE_NAME;
    $key_uri = self::PROPERTY_PHORGE_URI;
  ...
  public function extendEditForm(
    ...
    $v_name = $values[self::PROPERTY_PHORGE_NAME];
    if ($is_setup) {
      $e_name = idx($issues, self::PROPERTY_PHORGE_NAME, $e_required);
    } else {
      $e_name = null;
    }

    $v_uri = $values[self::PROPERTY_PHORGE_URI];
    $e_uri = idx($issues, self::PROPERTY_PHORGE_URI, $e_required);
      ...
      $form
       ->appendChild(
          id(new AphrontFormTextControl())
            ...
            ->setValue($v_name)
            ->setName(self::PROPERTY_PHORGE_NAME)
            ->setError($e_name)
            ->setCaption(pht(
    ...
    $form
      ->appendChild(
        id(new AphrontFormTextControl())
          ...
          ->setValue($v_uri)
          ->setName(self::PROPERTY_PHORGE_URI)
          ->setCaption(
            pht(
