...
,
    ...
    return array(
      $this->newOption('phorge.base-uri', 'string', null)
        ->setLocked(true)
      ...
      $this->newOption('phorge.production-uri', 'string', null)
        ->setSummary(
        ...
        ->setDescription(
          pht(
            ...
            'phorge.base-uri'))
        ->addExample('http://devtools.example.com/', pht('Valid Setting')),
      $this->newOption('phorge.allowed-uris', 'list<string>', array())
        ->setLocked(true)
        ...
        ->setDescription(
          pht(
            ...
            "across domains."))
        ->addExample(
          "http://phorge2.example.com/\n".
          "http://phorge3.example.com/",
          pht('Valid Setting')),
      $this->newOption('phorge.timezone', 'string', null)
        ->setSummary(
      ...
      $this->newOption('phorge.cookie-prefix', 'string', null)
        ->setLocked(true)
        ->setSummary(
          pht(
      ...
      $this->newOption('phorge.show-prototypes', 'bool', false)
        ->setLocked(true)
        ->setBoolOptions(
          array(
        ...
        ->setDescription(
          pht(
            ...
            $proto_doc_href,
            $proto_doc_name)),
      $this->newOption('phorge.serious-business', 'bool', false)
        ->setBoolOptions(
          array(
      ...
      $this->newOption('phorge.application-settings', 'wild', array())
        ->setLocked(true)
        ->setDescription(
          pht('Customized settings for applications.')),
      $this->newOption('phorge.cache-namespace', 'string', 'phorge')
        ->setLocked(true)
        ->setDescription(pht('Cache namespace.')),
      $this->newOption('phorge.silent', 'bool', false)
        ->setLocked(true)
        ->setBoolOptions(
          array(
            pht('Run Silently'),
  ...
  protected function didValidateOption(
    ...
    $key = $option->getKey();
    if ($key == 'phorge.base-uri' ||
        $key == 'phorge.production-uri') {
