...
final class PhabricatorObjectSelectorDialog extends Phobject {
  ...
  public function buildDialog() {
    ...
    require_celerity_resource('phorge-object-selector-css');
    ...
    if ($this->instructions) {
      $instructions = phutil_tag(
        'p',
        array('class' => 'phorge-object-selector-instructions'),
        $this->instructions);
    }

    $search_box = phorge_form(
      $user,
      array(
        'method' => 'POST',
      ...
      phutil_tag(
        'table',
        array('class' => 'phorge-object-selector-search'),
        phutil_tag('tr', array(), array(
          phutil_tag(
            'td',
            array('class' => 'phorge-object-selector-search-filter'),
            phutil_tag('select', array('id' => $filter_id), $options)),
          phutil_tag(
            'td',
            array('class' => 'phorge-object-selector-search-text'),
    ...
    $result_box = phutil_tag(
      'div',
      array(
        'class' => 'phorge-object-selector-results',
        'id' => $results_id,
      ),
      '');

    $attached_box = phutil_tag_div(
      'phorge-object-selector-current',
      phutil_tag_div(
        'phorge-object-selector-currently-attached',
        array(
          phutil_tag_div('phorge-object-selector-header', $this->header),
    ...
    $dialog
      ->setUser($this->user)
      ->setTitle($this->title)
      ->setClass('phorge-object-selector-dialog')
