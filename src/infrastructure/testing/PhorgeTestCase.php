...
abstract class PhabricatorTestCase extends PhutilTestCase {
   ...
   * side effects.
   */
  const PHORGE_TESTCONFIG_ISOLATE_LISK           = 'isolate-lisk';
   ...
   */
  const PHORGE_TESTCONFIG_BUILD_STORAGE_FIXTURES = 'storage-fixtures';
  ...
  private function getComputedConfiguration() {
    ...
    if ($config[self::PHORGE_TESTCONFIG_BUILD_STORAGE_FIXTURES]) {
      // Fixtures don't make sense with process isolation.
      $config[self::PHORGE_TESTCONFIG_ISOLATE_LISK] = false;
    }
  ...
  protected function willRunTests() {
    ...
    if ($config[self::PHORGE_TESTCONFIG_ISOLATE_LISK]) {
    ...
    $this->env->overrideEnvConfig('phorge.silent', false);
