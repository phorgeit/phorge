...
final class PhabricatorEnv extends Phobject {
  ...
  private static function initializeCommonEnvironment($config_optional) {
    ...
    $phorge_path = dirname(phutil_get_library_root('phorge'));
    $support_path = $phorge_path.'/support/bin';
    ...
    if (phutil_nonempty_string($instance)) {
      putenv('PHORGE_INSTANCE='.$instance);
    ...
    require_once $phorge_path.'/support/startup/preamble-utils.php';
  }
  ...
  private static function buildConfigurationSourceStack($config_optional) {
    ...
    // pull in all options from non-phorge libraries now they are loaded.
...
/* -(  Reading Configuration  )---------------------------------------------- */
  ...
  public static function getProductionURI($path) {
    ...
    $production_domain = self::getEnvConfig('phorge.production-uri');
  ...
  private static function getSelfURIMap() {
    ...
    $allowed_uris = self::getEnvConfig('phorge.allowed-uris');
  ...
  public static function getAnyBaseURI() {
    $base_uri = self::getEnvConfig('phorge.base-uri');
